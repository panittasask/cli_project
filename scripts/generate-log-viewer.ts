...
const { formatThinkingDetails } = require("../cli/logViewerPresentation") as {
    formatThinkingDetails: (response: Record<string, unknown>) => string;
};
...
function generateLogViewer(root = path.resolve(__dirname, "..")): string {
...
const responseFiles = fs.existsSync(logsDirectory)
    ...
    : [];
const responses = responseFiles.flatMap((name) => readJsonl(path.join(logsDirectory, name))).map((response) => {
    const { reasoningContent: _reasoningContent, ...rest } = response;
    return { ...rest, thinkingHtml: formatThinkingDetails(response) };
});
...
.tag.bad{background:#4b2930;color:#ffc3c7}summary{cursor:pointer;user-select:none}.thinking{border:1px solid #29496d;border-radius:7px;background:#111a27;padding:9px 11px}.thinking[open]{border-color:var(--accent)}.thinking summary{color:#b9d9ff;font-weight:600}.thinking pre{margin-bottom:2px;max-height:520px}
</style>
...
const esc=s=>String(s??'').replace(/[&<>]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[c]));const json=value=>{try{return typeof value==='string'?JSON.parse(value):value}catch{return undefined}};const pretty=x=>esc(JSON.stringify(x,null,2));const prettyJson=value=>{const parsed=json(value);return parsed===undefined?esc(value):pretty(parsed)};
...
function observationView(event){const observation=event.observation;const local=localSearchSummary(event);if(local)return local+'<details><summary>Raw tool data</summary><pre>'+prettyJson(observation)+'</pre></details>';const web=webSummary(observation);return web?web+'<details><summary>Raw tool data</summary><pre>'+prettyJson(observation)+'</pre></details>':'<details><summary>Tool result / observation</summary><pre>'+prettyJson(observation)+'</pre></details>'}
function thinkingView(response){return typeof response.thinkingHtml==='string'?response.thinkingHtml:''}
...
if (require.main === module) {
    console.log(`Log viewer written: ${generateLogViewer()}`);
}

module.exports = { generateLogViewer };
