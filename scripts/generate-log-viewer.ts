...
function observationView(observation){const web=webSummary(observation);return web?web+'<details><summary>Raw tool data</summary><pre>'+esc(observation)+'</pre></details>':'<details><summary>Tool result / observation</summary><pre>'+esc(observation)+'</pre></details>'}
...
function showTask(id){const rows=byTask.get(id)||[];const rs=responses.get(id)||[];let out='<h2>'+esc(id)+'</h2><div class="sub">'+esc(rows[0]?.timestamp)+' — '+esc(rows.at(-1)?.timestamp)+'</div>';for(const e of rows){const cls=e.status==='error'?'error':e.status==='ok'?'ok':'';out+='<div class="event"><b class="'+cls+'">Turn '+esc(e.turn)+' · '+esc(e.action||e.status)+'</b>'+(e.reason?'<div>'+esc(e.reason)+'</div>':'')+(e.arguments?'<details><summary>Tool request</summary><pre>'+pretty(e.arguments)+'</pre></details>':'')+(e.observation?observationView(e.observation):'')+'</div>'}out+='<h3>LLM responses ('+rs.length+')</h3>'+rs.map(r=>{const action=json(r.rawContent);const usage=r.usage||{};return '<div class="event"><b>Turn '+esc(r.turn)+' · '+esc(r.parsedAction||'parse error')+'</b>'+(action?.reason?'<div>'+esc(action.reason)+'</div>':'')+'<div class="meta">'+esc(r.durationMs)+' ms · prompt '+esc(usage.prompt_tokens)+' · output '+esc(usage.completion_tokens)+'</div><details><summary>Raw model response</summary><pre>'+esc(r.rawContent)+'</pre></details></div>'}).join('');document.querySelector('#detail').innerHTML=out}
