...
function renderList(){const q=document.querySelector('#filter').value.toLowerCase();const list=document.querySelector('#list');const tasks=[...byTask.entries()].sort((a,b)=>String(b[1].at(-1)?.timestamp).localeCompare(String(a[1].at(-1)?.timestamp)));let out='<h3>Execution tasks ('+tasks.length+')</h3>';for(const[id,rows]of tasks){const text=JSON.stringify(rows).toLowerCase();if(!text.includes(q)&&!id.includes(q))continue;const start=rows[0],last=rows.at(-1);out+='<button class="item" onclick="showTask('+JSON.stringify(id)+')"><b>'+esc(id)+'</b><div class="meta">'+esc(start?.timestamp)+' · '+rows.length+' events · '+esc(last?.action||last?.status)+'</div></button>'}const ss=Array.isArray(data.sessions.sessions)?data.sessions.sessions:[];out+='<h3>Conversation sessions ('+ss.length+')</h3>';for(const s of ss){const text=JSON.stringify(s).toLowerCase();if(!text.includes(q))continue;out+='<button class="item" onclick="showSession('+JSON.stringify(s.id)+')"><b>'+esc(s.title||s.id)+'</b><div class="meta">'+esc(s.id)+' · '+(s.messages||[]).length+' messages</div></button>'}list.innerHTML=out}
