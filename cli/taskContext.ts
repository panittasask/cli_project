...
const continuationPattern = /\b(it|that|this|those|these|continue|resume|carry on|also|same|previous)\b|(?:อันนี้|อันนั้น|เมื่อกี้|ต่อเลย|ทำ(?:งาน)?\s*ต่อ|ทำงาน\s*เก่า\s*ต่อ|ทำเลย|ยังไม่มี|ต่อ(?:จาก|จามก?)\s*(?:งาน)?เดิม|งาน\s*(?:เดิม|เก่า)|จากเดิม|เหมือนเดิม|ด้วยนะ|แล้วทีนี้|ที่ถาม)/i;
