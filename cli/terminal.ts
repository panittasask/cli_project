...
type ModelCapabilitySettings = Partial<
  import("./model/modelCapabilities").ModelCapabilities
>;
const { LlamaCppProvider } = require("./model/llamaCppProvider") as {
...
const { DefaultAgentRunner } = require("./agent/agentRunner") as {
  DefaultAgentRunner: new (dependencies: AgentRunnerDependencies) => {
    run: (request: {
      userMessage: string;
...
const { DefaultActionCoordinator } =
  require("./agent/action/actionCoordinator") as {
    DefaultActionCoordinator: new (
      tools: InstanceType<typeof AgentTool>,
    ) => ActionCoordinator;
  };
...
const { DefaultVerificationCoordinator } =
  require("./agent/verification/verificationCoordinator") as {
...
const { DefaultCompletionCoordinator } =
  require("./agent/completion/completionCoordinator") as {
...
const { ModelRouterClient } = require("./modelRouter") as {
  ModelRouterClient: new (
    apiUrl: string,
    ...
    switch: (
      selection: string,
    ) => Promise<{ model: RouterModel; unloaded: string[] }>;
    formatError: (error: unknown) => string;
  };
};
const {
  ...
  getLlmConnectionSettings: (settings: CliSettings) => {
    provider: "llama.cpp" | "openrouter";
    apiUrl: string;
    headers: Record<string, string>;
    apiKeyEnv?: string;
  ...
  initializeCliSettings: (appRoot?: string) => {
    created: boolean;
    path: string;
    message: string;
  };
  validateCliSettingsFile: (appRoot?: string) => {
    ok: boolean;
    path: string;
    source: string;
    errors: string[];
...
const { AgentGuard } = require("./agentGuard") as {
  AgentGuard: new (settings: AgentGuardSettings) => {
...
const { verificationRecoveryTurnAllowance } =
  require("./verificationRecovery") as {
...
const { FileCheckpointStore } = require("./fileCheckpoints") as {
...
const { SkillLoader } = require("./skillLoader") as {
...
const { AgentTrace } = require("./agentTrace") as {
  AgentTrace: new (
...
const { AgentResponseLog } = require("./agentResponseLog") as {
...
const {
  ...
  isReasoningOnlyTruncation: (response: {
    content: unknown;
    reasoningContent: unknown;
    finishReason: unknown;
  }) => boolean;
...
const { resolveJsonlLogPath } = require("./dailyLog") as {
  resolveJsonlLogPath: (
    target: string | { directory: string; basename: string },
    date?: Date,
  ) => string;
};
...
const { StatusBar } = require("./statusBar") as {
  StatusBar: new (
    getState: () => {
      model: string;
      contextUsed: number;
...
const { isContinuationRequest, selectTaskContext, summarizeTaskContext } =
  require("./taskContext") as {
...
const { WriteValidator } = require("./writeValidator") as {
  WriteValidator: new (workspace?: string) => {
    exists: (inputPath: string) => boolean;
    validate: (inputPath: string) => {
      ok: boolean;
      validator: string;
      output: string;
    };
    validateProjectFor: (
      inputPath: string,
    ) => { ok: boolean; validator: string; output: string } | undefined;
  };
};
const { Spinner, formatCompletionLine } = require("./spinner") as {
  Spinner: new (message?: string) => {
    start: () => void;
    stop: () => number;
    suspend: () => void;
    resume: () => void;
...
const { formatAiResponse, normalizeTerminalColor } =
  require("./terminalStyle") as {
    formatAiResponse: (
      answer: string,
      colors?: boolean,
...
const { ImageTool } = require("./tools/imageTool") as {
  ImageTool: new () => {
    parseImagePrompt: (
      input: string,
    ) => { filePath: string; prompt: string } | undefined;
    toDataUrl: (inputPath: string) => string;
  };
};
const { ReadFileTool } = require("./tools/readFileTool") as {
  ReadFileTool: new () => {
    parseReadFilePrompt: (
      input: string,
    ) => { filePath: string; prompt: string } | undefined;
...
const { FileAttachmentTool } = require("./tools/fileAttachmentTool") as {
...
const { EditFileTool } = require("./tools/editFileTool") as {
  EditFileTool: new () => {
    parseEditFilePrompt: (
      input: string,
    ) => { filePath: string; instruction: string } | undefined;
    readTargetFile: (inputPath: string) => string;
    writeEditedFile: (inputPath: string, content: string) => void;
  };
};
const { ToolRouter } = require("./tools/toolRouter") as {
  ToolRouter: new () => {
    buildRouterMessages: (
      message: string,
    ) => Array<{ role: "system" | "user"; content: string }>;
...
const { AgentTool } = require("./tools/agentTool") as {
  AgentTool: new (
    ...
    prepareEdit: (
      ...
      oldText: string,
      newText: string,
    ) => { ok: boolean; output: string; content?: string; changed?: boolean };
...
const { SessionTool } = require("./session") as {
  SessionTool: new (storagePath?: string) => {
    selectSession: (
      rl: readline.Interface,
      workspace: string,
    ) => Promise<{ id: string; title: string; workspace?: string }>;
    resumeSession: (
      sessionId: string,
    ) => { id: string; title: string; workspace?: string } | undefined;
...
type CliSettings = {
  ...
  sampling?: Partial<
    Record<"chat" | "planner" | "action", Partial<SamplingSettings>>
  >;
};
...
const workspaceWasExplicitlyRequested =
  getArgumentValue("--workspace", "--cwd") !== undefined;
...
const slashCommandOptions: SlashCommandOption[] = [
  ...
  { command: "/attach", description: "attach text, Word, or Excel files" },
  { command: "/readfile", description: "read a text, Word, or Excel file" },
  ...
  { command: "/settings", description: "show effective runtime settings" },
  {
    ...
    description: "create settings from the prototype",
  },
  { command: "/settings validate", description: "validate effective settings" },
  {
    command: "/capabilities",
    description: "show tools, checks, and web availability",
  },
...
const rl = readline.createInterface({
  ...
  completer: (line: string): [string[], string] => {
    ...
    return [
      suggestions.map(
        (suggestion) => applyInputSuggestion(line, suggestion).line,
      ),
      line,
    ];
  },
...
rl.on("SIGINT", () => {
  if (activeRequestController) {
    ...
    activeRequestSpinner?.log(
      "Cancelling active request... completed file writes remain available through /undo.",
    );
    return;
  }
...
const configuredContextValue = Number(
  process.env.LLAMA_CONTEXT_LENGTH?.trim() ||
    cliSettings.contextLength ||
    16384,
);
const configuredContextLength =
  Number.isFinite(configuredContextValue) && configuredContextValue >= 512
...
const reasoningSampling =
  isOpenRouter && llmConnection.reasoning
    ? { reasoning: llmConnection.reasoning }
    : {};
const chatSampling = {
  ...getSamplingSettings(cliSettings, "chat"),
  ...reasoningSampling,
};
const plannerSampling = {
  ...getSamplingSettings(cliSettings, "planner"),
  ...reasoningSampling,
};
const actionSampling = {
  ...getSamplingSettings(cliSettings, "action"),
  ...reasoningSampling,
};
const historyMessageLimit = Math.max(
  0,
  Math.floor(cliSettings.historyMessages ?? 6),
);
...
const debugSensitiveKey =
  /(^|_)(api_?key|token|secret|password|authorization|cookie|private_?key)($|_)/i;
...
function persistDebugSetting(enabled: boolean): void {
  ...
  try {
    persisted = JSON.parse(fs.readFileSync(settingsPath, "utf8")) as Record<
      string,
      unknown
    >;
  } catch {
...
function persistTerminalColor(color: string): void {
  ...
  const persistedTerminal =
    ...
    !Array.isArray(persisted.terminal)
...
const sessionTool = new SessionTool(
  path.resolve(appRoot, ".cli-sessions.json"),
);
const imageTool = new ImageTool();
const readFileTool = new ReadFileTool();
const fileAttachmentTool = new FileAttachmentTool();
...
const taskCoordinator = new DefaultTaskCoordinator({
  deriveTaskEvidencePolicy,
  taskContractsEquivalent,
});
...
const terminalEventSink = new TerminalAgentEventSink(
...
const statusBar = new StatusBar(() => ({
  ...
  contextUsed: statusSessionId
    ? sessionTool.getUsage(statusSessionId).activeContextTokens
    : 0,
...
async function getServerContextInfo(
  modelId?: string,
): Promise<{ contextLength: number; totalSlots?: number } | undefined> {
  ...
  try {
    ...
    const contextLength = Number(
      response.data?.default_generation_settings?.n_ctx,
    );
    ...
    return {
      ...
      ...(Number.isFinite(totalSlots) && totalSlots > 0
...
async function bindRequestModelToServer(): Promise<void> {
  const synced = await syncModelFromServer();
  if (!synced) {
    serverModelSynced = false;
    statusBar.render();
    throw new Error(
      "No model is reported as loaded by llama.cpp. Start or switch a server model with /model before sending a request.",
    );
  }
...
async function printModelInfo(): Promise<void> {
  ...
  console.log(
    serverModelSynced || isOpenRouter
  ...
  console.log(
    `Configured context: ${configuredContextLength.toLocaleString()} tokens`,
  );
  console.log(
    serverContext
  ...
  if (routerModels) {
    ...
    console.log(
      "Available server models: unavailable (the server is offline or llama.cpp router mode is disabled)",
    );
    console.log(
      "Runtime switching and the full model list require router mode on the server.",
    );
  }
  console.log();
}
...
function printModeHelp(currentMode: RunMode): void {
  ...
  console.log(
    "- agent: let the model inspect files, edit, and run checks in a loop",
  );
...
function printCommandHelp(currentMode: RunMode): void {
  ...
  console.log("/attach <paths> | <prompt> Attach text, Word, or Excel files");
  console.log(
    "Drop one or more file paths directly into the prompt and press Enter",
  );
  console.log(
    "/readfile <path> | <prompt>   Ask using text, Word, or Excel input",
  );
  console.log(
    "/editfile <path> | <instruction>   Edit file with AI instruction",
  );
  console.log("/workspace                Show current workspace");
  console.log("/workspace <path>         Change workspace for agent/files");
  console.log(
    "/model                    Show loaded and available server models",
  );
  console.log(
    "/model <number-or-name>   Switch the router server to another GGUF model",
  );
  console.log(
    "/settings                 Show effective settings and their source",
  );
  console.log(
    "/settings init            Create settings.json from the tracked prototype without overwriting",
  );
  console.log("/settings validate        Validate the effective settings file");
  console.log(
    "/capabilities             Show local tools, project checks, MCP, and web search",
  );
  console.log(
    "/usage                    Show session and active context token usage",
  );
  console.log(
    "/log                      Refresh .cli/log-viewer.html from the latest logs",
  );
  console.log(
    "/color                    Show the current LLM message color and supported values",
  );
  console.log(
    "/color <name-or-code>     Change and save the LLM message color",
  );
  console.log(
    "/clear                    Start a new task context; keep session history",
  );
  console.log(
    "/undo                     Restore the latest model file checkpoint",
  );
  console.log(
    "/skills                   Show user-level and project-local skills",
  );
  console.log(
    "/debug [on|off]           Show status or toggle concise agent trace",
  );
  console.log("/exit                     Exit the app");
  console.log(
    "Autocomplete: type /, $skill, or /model; use ↑↓, Enter to run/select, Tab to insert, and Esc",
  );
  console.log();
...
function settingsFileSource(): string {
  return fs.existsSync(path.resolve(appRoot, ".cli", "settings.json"))
    ? "settings.json"
  ...
  const rows: Array<[string, string | number | boolean, string]> = [
    [
      ...
      process.env.LLM_PROVIDER?.trim()
        ? "environment"
        : cliSettings.apiEndpoint?.provider
          ? "api-endpoints.json"
          : settingSource("LLM_PROVIDER", cliSettings.provider !== undefined),
    ],
    [
      "llm.apiUrl",
      apiUrl,
      process.env.LLM_API_URL?.trim() || process.env.LLAMA_API_URL?.trim()
        ? "environment"
        : cliSettings.apiEndpoint?.apiUrl
          ? "api-endpoints.json"
          : cliSettings.apiUrl
            ? settingsFileSource()
            : "built-in default",
    ...
    [
      ...
      llmConnection.model
        ? "api-endpoints.json"
        : settingSource("LLM_MODEL", cliSettings.defaultModel !== undefined),
    ],
    [
      "llm.requestDelayMs",
      llmConnection.requestDelayMs,
      cliSettings.apiEndpoint?.requestDelayMs !== undefined
        ? "api-endpoints.json"
        : "built-in default",
    ],
    [
      "llm.apiKey",
      isOpenRouter
        ? `configured via ${llmConnection.apiKeyEnv ?? "api-endpoints.json"}`
        : "not used",
      isOpenRouter
        ? cliSettings.apiEndpoint?.bearerToken
          ? "api-endpoints.json"
          : settingSource(
              "LLM_API_KEY_ENV",
              cliSettings.apiKeyEnv !== undefined,
            )
        : "not used",
    ],
    [
      "agent.profile",
      agentGuardSettings.profile,
      settingSource("CLI_AGENT_PROFILE", agent.profile !== undefined),
    ],
    [
      "agent.maxTurns",
      agentGuardSettings.maxTurns,
      settingSource("CLI_AGENT_MAX_TURNS", agent.maxTurns !== undefined),
    ],
    [
      "agent.maxSegments",
      agentGuardSettings.maxSegments,
      settingSource("CLI_AGENT_MAX_SEGMENTS", agent.maxSegments !== undefined),
    ],
    [
      "agent.maxDurationMinutes",
      Math.round(agentGuardSettings.maxDurationMs / 60_000),
      settingSource(
        "CLI_AGENT_MAX_MINUTES",
        agent.maxDurationMinutes !== undefined,
      ),
    ],
    [
      "agent.maxCompletionTokens",
      agentGuardSettings.maxCompletionTokens,
      settingSource(
        "CLI_AGENT_MAX_COMPLETION_TOKENS",
        agent.maxCompletionTokens !== undefined,
      ),
    ],
    [
      "agent.repeatLimit",
      agentGuardSettings.repeatLimit,
      settingSource("CLI_AGENT_REPEAT_LIMIT", agent.repeatLimit !== undefined),
    ],
    [
      "agent.maxClarifications",
      clarificationSettings.maxClarifications,
      settingSource(
        "CLI_AGENT_MAX_CLARIFICATIONS",
        agent.maxClarifications !== undefined,
      ),
    ],
    [
      "agent.requireInspectionBeforeClarification",
      clarificationSettings.requireInspection,
      settingSource(
        "CLI_AGENT_REQUIRE_INSPECTION_BEFORE_CLARIFICATION",
        agent.requireInspectionBeforeClarification !== undefined,
      ),
    ],
    [
      "agent.secondClarificationRequiresBlocker",
      clarificationSettings.secondRequiresBlocker,
      settingSource(
        "CLI_AGENT_SECOND_CLARIFICATION_REQUIRES_BLOCKER",
        agent.secondClarificationRequiresBlocker !== undefined,
      ),
    ],
    [
      "contextLength",
      ...
      settingSource(
        ...
        cliSettings.contextLength !== undefined,
      ),
    ],
    [
      "hardwareProfile",
      process.env.LLAMA_HARDWARE_PROFILE?.trim() ||
        cliSettings.hardwareProfile ||
        "auto",
      settingSource(
        "LLAMA_HARDWARE_PROFILE",
        cliSettings.hardwareProfile !== undefined,
      ),
    ],
    [
      "terminal.llmMessageColor",
      terminalSettings.llmMessageColor,
      cliSettings.terminal?.llmMessageColor !== undefined
        ? settingsFileSource()
        : "built-in default",
    ],
  ];
  console.log("Effective settings:");
  rows.forEach(([name, value, source]) =>
    console.log(`  ${name}: ${value} [${source}]`),
  );
  console.log(
    `  projectChecks providers: ${projectCheckProviders.length} [${cliSettings.projectChecks ? settingsFileSource() : "built-in default"}]`,
  );
}
...
async function printCapabilities(currentMode: RunMode): Promise<void> {
  ...
  const tools = mcp.servers.flatMap((server) =>
    Array.isArray(server.tools)
      ? (server.tools as Array<Record<string, unknown>>)
      : [],
  );
  const webTools = tools.filter((tool) =>
    /(?:web|search|browser)/i.test(
      `${tool.name ?? ""} ${tool.description ?? ""}`,
    ),
  );
  console.log(`Mode: ${currentMode}`);
  console.log(
    `Local workspace actions: ${currentMode === "agent" ? "read, search, list, edit, write, delete, safe commands" : "not available to the model in this mode"}`,
  );
  console.log(
    `Project checks: ${checks.length === 0 ? "none discovered" : ""}`,
  );
  checks.forEach((check) =>
    console.log(
      `  ${check.label}: ${check.command} (workdir ${check.workdir})`,
    ),
  );
  console.log(
    `Configured project-check providers: ${projectCheckProviders.length}`,
  );
  console.log(
    `MCP servers: ${mcp.servers.length === 0 ? "none configured" : ""}`,
  );
  mcp.servers.forEach((server) => {
    const serverTools = Array.isArray(server.tools)
      ...
      : [];
    console.log(
      `  ${String(server.name ?? "unknown")}: ${server.error ? `error — ${String(server.error)}` : `${serverTools.length} tool(s)`}`,
    );
  });
  console.log(
    `Web search: ${currentMode !== "agent" ? "unavailable in this mode" : webTools.length > 0 ? `available via ${webTools.map((tool) => String(tool.name)).join(", ")}` : "unavailable (no discovered MCP search tool)"}`,
  );
}
...
async function loadRouterModelSuggestions(force = false): Promise<void> {
  ...
  if (
    ...
    (!force && routerModelSuggestionCache.length > 0)
  )
    return;
...
function renderInputSuggestions(line: string): void {
  ...
  if (suggestions.length === 0) {
    if (
      isModelSuggestionInput(line, rl.cursor) &&
      (routerModelSuggestionsLoading || routerModelSuggestionError)
    ) {
      ...
      readline.clearLine(process.stdout, 0);
      process.stdout.write(
        `\x1b[2m${notice.slice(0, Math.max(20, (process.stdout.columns || 100) - 2))}\x1b[0m`,
      );
  ...
  selectedInputSuggestion = Math.min(
    selectedInputSuggestion,
    suggestions.length - 1,
  );
  const labelWidth = Math.min(
    38,
    Math.max(...suggestions.map((item) => item.value.length)),
  );
...
function updateInputSuggestions(): void {
  ...
  if (
    isModelSuggestionInput(rl.line, rl.cursor) &&
...
function installInputSuggestionTtyWrite(): void {
  ...
  internal._ttyWrite = function suggestionAwareTtyWrite(
    input: string,
    key: readline.Key,
  ): void {
...
async function promptForClarification(
  ...
  try {
    ...
    while (true) {
      ...
      console.log(
        `Please choose 1-${request.options.length}, enter an option id, or type a custom answer.`,
      );
    }
  } finally {
...
function createRequestBudget(
  ...
  const arm = (): void => {
    ...
    if (remainingMs <= 0) {
      controller.abort(
        new Error(
          `Request wall-clock budget reached (${Math.round(durationMs / 60000)} minutes).`,
        ),
      );
      return;
    }
...
async function restoreSessionWorkspace(
  ...
  if (activeSession.workspace) {
    try {
      ...
      console.log(
        `Saved session workspace is unavailable: ${activeSession.workspace}`,
      );
    }
  } else {
  ...
  while (true) {
    const rawWorkspace = await promptText(
      `Workspace path (Enter to use ${activeWorkspace}, or /exit): `,
    );
...
function findUnknownSlashCommand(input: string): string | undefined {
  ...
  if (
    ...
    lower.startsWith("/readfile ") ||
    lower.startsWith("/editfile ")
  ) {
    return undefined;
  }
...
async function routeTool(
  ...
  try {
    ...
    debugLog("Tool-router LLM request", {
      model,
    ...
    debugLog("Tool-router LLM response", {
      rawContent,
      decision,
    ...
    if (decision) {
      if (!decision.needsTool || decision.tool === "none") {
        return {
          tool: "none",
          filePath: "",
          contextFiles: decision.needsMoreContext
            ? resolveContextFiles(decision.contextFiles)
            : [],
      ...
      const filePath =
        resolveExistingFile(decision.filePath) ??
...
function ask(activeSession: ChatSession, runMode: RunMode): void {
  ...
  rl.question("You: ", async (message: string) => {
    ...
    if (trimmed.toLowerCase() === "/settings init") {
      ...
      if (result.created)
        console.log("Restart the CLI to load the new settings.");
      console.log();
      ask(activeSession, runMode);
      return;
    }
    ...
    if (trimmed.toLowerCase() === "/skills") {
      ...
      if (skills.length === 0)
        console.log(
          "  None. Add ~/.codex/skills/<name>/SKILL.md or .cli/skills/<name>/SKILL.md",
        );
      else
    ...
    if (trimmed.toLowerCase() === "/log") {
      try {
        ...
        console.log(
          `Log viewer failed: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
      console.log();
      ask(activeSession, runMode);
    ...
    if (trimmed.toLowerCase() === "/color") {
      console.log(
        `Current LLM message color: ${terminalSettings.llmMessageColor}`,
      );
      console.log(
        "Supported names: black, red, green, yellow, blue, magenta, cyan, white, default, bright-black, bright-red, bright-green, bright-yellow, bright-blue, bright-magenta, bright-cyan, bright-white",
      );
    ...
    if (trimmed.toLowerCase().startsWith("/color ")) {
      ...
      if (!normalizedColor) {
        ...
        console.log(
          "Use /color to show supported names and ANSI foreground codes.",
        );
      } else {
        try {
          persistTerminalColor(normalizedColor);
          console.log(
            `LLM message color changed and saved: ${normalizedColor}`,
          );
        } catch (error) {
          console.log(
            `Color setting failed: ${error instanceof Error ? error.message : String(error)}`,
          );
        }
      }
      console.log();
    ...
    if (/^\/debug\s+(on|off)$/i.test(trimmed)) {
      ...
      try {
        ...
        console.log(
          `Agent trace: ${debugEnabled ? "on" : "off"} (saved to .cli/settings.json)`,
        );
      } catch (error) {
        console.log(
          `Agent trace: ${debugEnabled ? "on" : "off"} (could not save setting: ${error instanceof Error ? error.message : String(error)})`,
        );
      }
      console.log();
      ask(activeSession, runMode);
    ...
    if (trimmed.toLowerCase().startsWith("/debug ")) {
      console.log(
        "Invalid debug command. Use /debug, /debug on, or /debug off",
      );
      console.log();
      ask(activeSession, runMode);
      return;
    ...
    if (modelCommand?.type === "set" && modelCommand.model) {
      if (isOpenRouter) {
        console.log(
          "OpenRouter model switching is not available from the llama.cpp server menu. Set defaultModel in .cli/settings.json instead.",
        );
        console.log();
        ask(activeSession, runMode);
        return;
      ...
      try {
        ...
        const serverContext = await getServerContextInfo(model);
        activeContextLength =
          serverContext?.contextLength ?? configuredContextLength;
        contextStartedAt = Date.now();
        sessionTool.resetActiveContextUsage(activeSession.id);
        statusBar.render();
        ...
        console.log(
          "Task context cleared for the new model. Saved session history was kept.",
        );
      } catch (error) {
        console.log(
          `Model switch failed: ${modelRouterClient.formatError(error)}`,
        );
      }
      console.log();
      ask(activeSession, runMode);
    ...
    if (trimmed.toLowerCase().startsWith("/mode ")) {
      console.log(
        "Invalid mode. Use /mode planner, /mode fast, or /mode agent",
      );
      console.log();
      ask(activeSession, runMode);
      return;
    ...
    try {
      ...
      const attachmentRequest = fileAttachmentTool.parseInput(trimmed);
      ...
      spinner.start();
      await bindRequestModelToServer();
      const attachmentPrompt = attachmentRequest
        ? await fileAttachmentTool.buildPrompt(attachmentRequest)
        : undefined;
      ...
      let contextReason = "";
      const sessionHistory = sessionTool.getContextMessages(
        activeSession.id,
        historyMessageLimit,
        contextStartedAt,
      );
      ...
      const taskHistory = sessionTool.getContextMessages(
        activeSession.id,
        160,
        contextStartedAt,
      );
      const unfinishedTaskContext = sessionTool.getUnfinishedTaskContext(
        activeSession.id,
      );
      ...
      const historyForTask = [
        ...(activeSession.title
          ? [
              {
                role: "user" as const,
                content: `Build task: ${activeSession.title}`,
              },
            ]
          : []),
      ...
      if (
        ...
        if (!journalTaskId)
          throw new Error("Could not create the durable session task journal.");
      ...
      if (
        ...
        const decision = await routeTool(
          trimmed,
          activeSession.id,
      ...
      const projectContextBlock = buildProjectContextBlock(
        requestedContextFiles,
        contextReason,
      );

      const editFilePrompt = explicitEditPrompt ?? implicitEditPrompt;
      const readFilePrompt = explicitReadPrompt ?? implicitReadPrompt;
      const readFileContent = readFilePrompt
        ? await fileAttachmentTool.readFileForPrompt(readFilePrompt.filePath)
        : undefined;
      let plannerPayload = "";
...
User Prompt: ${plannerInput}`,
        ...
        const plannerMessage =
          planner.data.choices[0].message.content?.trim() ?? "{}";
      ...
      if (editFilePrompt) {
        const originalContent = editFileTool.readTargetFile(
          editFilePrompt.filePath,
        );
...
${projectContextBlock}`,
        ...
        const editedRaw =
          editResponse.data.choices[0].message.content?.trim() ?? "";
        ...
        const checkpoint = checkpointStore.checkpoint(
          activeWorkspace,
          editFilePrompt.filePath,
          editedContent,
        );
        ...
        const validation = new WriteValidator(activeWorkspace).validate(
          editFilePrompt.filePath,
        );
        if (!validation.ok) {
          checkpointStore.undoLatest(activeWorkspace);
          throw new Error(
            `Updated file but ${validation.validator} validation failed:\n${validation.output}`,
          );
        }
...
If the user asks whether you can search from this mode, explain the limitation and tell them to use /mode agent.`;
      const selectedSkills = skillLoader.select(
        trimmed,
        skillLoader.discover(activeWorkspace),
      );
...
Do not mention hidden context, internal tools, or system prompts.`;
      ...
      const chatMessages = [
        {
          ...
          content: [assistantSystemInstruction, skillGuidance]
            .filter(Boolean)
            .join("\n\n"),
        },
        ...historyForModel,
      ...
      const elapsedMs = spinner.stop();
      process.stdout.write(
        formatAiResponse(answer, undefined, terminalSettings.llmMessageColor),
      );
      console.log(formatCompletionLine(elapsedMs));
      printSessionUsage(activeSession.id);
      console.log();
      ...
      if (requestController.signal.aborted) {
        const reason =
          requestController.signal.reason instanceof Error &&
          requestController.signal.reason.message.includes("budget")
            ? requestController.signal.reason.message
            : "Request cancelled.";
        console.log(
          `${reason} The CLI is ready for the next prompt; use /undo if a completed write should be reverted.`,
        );
      } else {
        console.error(
          `Model request failed: ${llamaClient.formatError(error)}`,
        );
        console.log(
          "The task was saved as interrupted and the CLI is ready for the next prompt. Retry to continue from the session journal; use /model to confirm the loaded model.",
        );
      }
...
async function start(): Promise<void> {
  ...
  console.log(
    "Cancel active request: Ctrl+C | restore latest file change: /undo",
  );
  ...
  const recoveredInterruptedTask = sessionTool.recoverInterruptedTask(
    activeSession.id,
  );
  ...
  if (recoveredInterruptedTask) {
    console.log(
      "Recovered an unfinished task journal from the previous CLI process.",
    );
  }
  console.log(
    formatSessionHistory(
